...
def registro():
    ...
    if request.method=='POST':
        ...
        #---------------consultar sobre marca de tiempo------------------------
            ...
            consultas.insertar_registro_a_db(username=username, email=email, password=password, phone=phone, description=description, address=address, image="/static/cambiar_avatar/"+nombre_imagen)  
...
@app.route("/cambiar_contraseña/<token>")
...
#---------------------------------------------------------------------------------------
@app.route("/cambiar_contraseña/actualizada/<email>", methods=["GET", "POST"])
def cambiarP(email):
    if request.method == "POST":
        password = request.form["password"]
        ...
        if password == password_confirm:
            if password == "" or password_confirm == "":
                ...
                flash("Los campos de las credenciales son requeridos")
                return render_template("recuperar_contraseña2.html")
        else:
...
@app.route("/user_home_user", methods=["GET","POST"])
def user_home_user():
    return render_template("user_home.html")
...
def company_products():
    ...
    return render_template("menu_empresa.html")

@app.route("/productos_empresa/<int:id>", methods=['GET','POST'])
