...
def insertar_registro_a_db(username, email, password, phone, description, address, image):
    cursor = db.cursor(buffered=True)
    cursor.execute("INSERT INTO login_users (username, email, password, phone, description, address, image) values (%s,%s,%s,%s,%s,%s,%s)",(
    username, 
    email, 
    password, 
    phone, 
...
def mostrar_crud(nombre, descripción, precio, estado, imagen):
    cursor = db.cursor(dictionary=True,buffered=True)
...
def email_no_repetido(email):
    ...
    email_verified = cursor.fetchone()
